...
<?else:?>
    <div class="container simpleform-wrapper">
        <div class="row">
            <?if ($arResult['result'] > 0):?>
                <a class="form-success-btn" id="contactsFormSuccessTrigger" href="#contactsFormSuccess"></a>
                <div class="d-none">
                    <div class="fb-modal fb-modal--form-success" id="contactsFormSuccess">
                        <div class="fb-modal-inner">
                            <p><?=GetMessage('PK_FORM_SIMPLE_RENT_FORM_SUCCESS_MESSAGE')?></p>
                        </div>
                    </div>
                </div>
                <script>
                    $('body').click();
                    $('body').find('#contactsFormSuccessTrigger').click().click();
                </script>
            <?endif?>

            <div class="col-12 col-lg-10 col-xl-8 offset-0 offset-lg-1 offset-xl-2">
                <h2><?=GetMessage('PK_FORM_SIMPLE_RENT_FORM_HEADING')?></h2>
                <div class="feedback-form-block">
                    <form class="form feedback-form simpleform simpleform-reload" method="post" enctype="multipart/form-data" action="#">
                        ...
                        <input type="hidden" name="template" value="rent_form">
                        <input type="hidden" name="IBLOCK_ID" value="<?=$arParams['IBLOCK_ID']?>">
                        <input type="hidden" name="CLASS" value="<?=$arParams['CLASS_ID']?>">
                            ...
                            <div class="field field--checkbox">
                                <label class="custom-checkbox-label<?=$arResult['error']['req_SOGLASIE'] ? ' custom-checkbox-label--error' : ''?>">
                                    ...
                                    <span class="custom-checkbox">
                                        <img class="image" src="<?=SITE_TEMPLATE_PATH?>/images/svg/checkbox-check.svg" alt="<?=GetMessage('PK_FORM_SIMPLE_RENT_FORM_AGREEMENT_CHECKBOX_IMG_ALT')?>">
                                    </span>
                                    <span class="custom-checkbox-text-label"><?=GetMessage('PK_FORM_SIMPLE_RENT_FORM_AGREEMENT_TEXT')?></span>
                                </label>
                            </div>
                        <?endif?>

                        <div class="btn-wrapper">
                            <button type="submit" class="green-btn"><?=GetMessage('PK_FORM_SIMPLE_RENT_FORM_SUBMIT_BTN')?></button>
                        </div>
                    </form>

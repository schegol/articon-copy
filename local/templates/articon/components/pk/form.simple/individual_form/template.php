...
<?if ($arParams['SHOW_TEXT'] == 'Y'):?>
    <section class="section-feedback form-replacement-text">
        <div class="container">
            <div class="row">
                <?$APPLICATION->IncludeComponent(
                    ...
                    Array(
                        ...
                        "AREA_FILE_SUFFIX" => "",
                        "EDIT_TEMPLATE" => "",
                        "PATH" => "/include/form_replacement_text.php"
                    )
                );?>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row">
                <?if ($arResult['result'] > 0):?>
                    <a class="form-success-btn" id="individualFormSuccessTrigger" href="#individualFormSuccess"></a>
                    <div class="d-none">
                        <div class="fb-modal fb-modal--form-success" id="individualFormSuccess">
                            <div class="fb-modal-inner">
                                <p><?=GetMessage('PK_FORM_SIMPLE_INDIVIDUAL_FORM_SUCCESS_MESSAGE')?></p>
                            </div>
                        </div>
                    </div>
                    <script>
                        $('body').click();
                        $('body').find('#individualFormSuccessTrigger').click().click();
                    </script>
                <?endif?>

                <div class="col-12 col-lg-10 col-xl-8 offset-0 offset-lg-1 offset-xl-2">
                    <h2><?=GetMessage('PK_FORM_SIMPLE_INDIVIDUAL_FORM_HEADING')?></h2>
                    <h4><?=GetMessage('PK_FORM_SIMPLE_INDIVIDUAL_FORM_SUBHEADING')?></h4>
                    ...
                    <div class="feedback-form-block">
                        <form class="form feedback-form simpleform simpleform-reload" method="post" enctype="multipart/form-data" action="#">
                            ...
                            <input type="hidden" name="template" value="individual_form">
                                ...
                                <div class="field field--checkbox">
                                    <label class="custom-checkbox-label<?=$arResult['error']['req_SOGLASIE'] ? ' custom-checkbox-label--error' : ''?>">
                                        ...
                                        <span class="custom-checkbox">
                                            <img class="image" src="<?=SITE_TEMPLATE_PATH?>/images/svg/checkbox-check.svg" alt="<?=GetMessage('PK_FORM_SIMPLE_INDIVIDUAL_FORM_AGREEMENT_CHECKBOX_IMG_ALT')?>">
                                        </span>
                                        <span class="custom-checkbox-text-label"><?=GetMessage('PK_FORM_SIMPLE_INDIVIDUAL_FORM_AGREEMENT_TEXT')?></span>
                                    </label>
                                </div>
                            <?endif?>

                            <div class="btn-wrapper">
                                <button type="submit" class="green-btn"><?=GetMessage('PK_FORM_SIMPLE_INDIVIDUAL_FORM_SUBMIT_BTN')?></button>
                            </div>
                        </form>
